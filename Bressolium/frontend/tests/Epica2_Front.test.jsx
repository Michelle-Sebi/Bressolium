...
vi.mock('../src/features/techtree/TechTreeModal', () => ({
    default: ({ isOpen, onClose }) => isOpen
        ? <div data-testid="tech-tree-modal"><button onClick={onClose}>Cerrar</button></div>
        : null,
}));
...
// ─────────────────────────────────────────────────────────────────────────────
// TESTS PARA: TAREA 51
// Título: Pueblo Tile — Center Placement + Tech Tree Access
// ─────────────────────────────────────────────────────────────────────────────

/**
 * Genera los 225 tiles del tablero con la casilla central (7,7) siendo de tipo pueblo.
 * La casilla pueblo está siempre explorada y sin jugador asignado (es compartida).
 */
function createMockTilesWithPueblo() {
    return createMockTiles().map((t) =>
        t.coord_x === 7 && t.coord_y === 7
            ? {
                  ...t,
                  explored:        true,
                  assigned_player: null,
                  type: { base_type: 'pueblo', level: 1, name: 'Pueblo' },
              }
            : t
    );
}

describe('T51 — Pueblo Tile: estilo visual diferenciado', () => {

    it('la casilla pueblo (7,7) tiene la clase tile--pueblo en el contenedor exterior', () => {
        mockReduxState({ tiles: createMockTilesWithPueblo() });
        renderComponent();

        const innerTile = screen.getByTestId('tile-7-7');
        expect(innerTile.parentElement).toHaveClass('tile--pueblo');
    });

    it('la casilla pueblo expone data-base-type="pueblo" cuando está explorada', () => {
        mockReduxState({ tiles: createMockTilesWithPueblo() });
        renderComponent();

        expect(screen.getByTestId('tile-7-7')).toHaveAttribute('data-base-type', 'pueblo');
    });

    it('las casillas que no son pueblo no tienen la clase tile--pueblo', () => {
        mockReduxState({ tiles: createMockTilesWithPueblo() });
        renderComponent();

        const bosqueTile = screen.getByTestId('tile-0-0');
        expect(bosqueTile.parentElement).not.toHaveClass('tile--pueblo');
    });

});

describe('T51 — Pueblo Tile: click abre modal del árbol tecnológico', () => {

    it('click en la casilla pueblo abre el modal del árbol tecnológico', () => {
        mockReduxState({ tiles: createMockTilesWithPueblo() });
        renderComponent();

        expect(screen.queryByTestId('tech-tree-modal')).not.toBeInTheDocument();
        fireEvent.click(screen.getByTestId('tile-7-7'));
        expect(screen.getByTestId('tech-tree-modal')).toBeInTheDocument();
    });

    it('click en la casilla pueblo NO llama a exploreTile', () => {
        mockReduxState({ tiles: createMockTilesWithPueblo() });
        renderComponent();
    ...
    it('click en la casilla pueblo NO llama a upgradeTile', () => {
        mockReduxState({ tiles: createMockTilesWithPueblo() });
        renderComponent();
    ...
    it('el modal del árbol tecnológico puede cerrarse', () => {
        mockReduxState({ tiles: createMockTilesWithPueblo() });
        renderComponent();
    ...
    it('click en una casilla de bosque no abre el modal', () => {
        mockReduxState({ tiles: createMockTilesWithPueblo() });
        renderComponent();
