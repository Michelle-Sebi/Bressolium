...
test('el diagrama ER existe en la ruta de documentación', function () {
    $path = diagramPath();
    expect(file_exists($path))->toBeTrue("No se encontró el diagrama ER en: $path");
});

test('el diagrama ER refleja la nueva columna base_type de tile_types (T21)', function () {
    $content = file_get_contents(diagramPath());
    expect($content)->toContain('base_type');
});

test('el diagrama ER refleja las columnas de exploración de tiles (T21)', function () {
    $content = file_get_contents(diagramPath());
    expect($content)->toContain('explored_by_player_id')
        ->and($content)->toContain('explored_at');
});

test('el diagrama ER refleja las columnas tier y group de materials (T21)', function () {
    $content = file_get_contents(diagramPath());
    expect($content)->toContain('tier')
        ->and($content)->toContain('group');
});

test('el diagrama ER refleja las nuevas tablas de prerequisitos (T22)', function () {
    $content = file_get_contents(diagramPath());
    expect($content)->toContain('invention_prerequisites')
        ->and($content)->toContain('technology_prerequisites');
});

test('el diagrama ER refleja la tabla invention_costs separada de recipes (T22)', function () {
    $content = file_get_contents(diagramPath());
    expect($content)->toContain('invention_costs');
});

test('el diagrama ER refleja las tablas de bonificadores (T22)', function () {
    $content = file_get_contents(diagramPath());
    expect($content)->toContain('technology_bonuses')
        ->and($content)->toContain('invention_bonuses');
});

test('el diagrama ER refleja las tablas de desbloqueos (T22)', function () {
    $content = file_get_contents(diagramPath());
